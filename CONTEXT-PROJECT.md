...
Ver STRATUM.md para contexto completo del proyecto. Este archivo queda solo como referencia historica.

# STRATUM — Contexto del Proyecto
> Última actualización: 2026-05-08
> Versión actual: **v0.31.0** — Pre-1.0 / WIP (EDA legacy DRC migration — Phase A: retiro de `electrical_drc.py` + `mcu_pinout_validator.py` 2026-05-08)
> Tagline: _"Tu memoria técnica siempre disponible"_
> Estado: **Pre-1.0 / WIP** — corre en local + Railway pero el API y el pipeline EDA aún están en consolidación; el objetivo es llegar a una base estable v1.0. Hasta entonces cualquier release puede romper compatibilidad.
>
> Nota: el changelog de abajo refleja v0.23.0, v0.27.0, v0.28.0, v0.29.0, v0.30.0 y v0.31.0; las versiones intermedias (v0.24/0.25/0.26) están documentadas en la memoria del proyecto pero aún no migradas aquí.

> **2026-05-08 — v0.31.0 EDA legacy DRC migration — Phase A (retiro de 2/3 módulos legacy):** consolida el pipeline IR retirando `tools/electrical_drc.py` (~600 LOC, 18 reglas DRC) y `tools/mcu_pinout_validator.py` (~400 LOC, validación de pines MCU). **Adaptador**: `tools/eda/ir/legacy.py::dict_to_ir(d) -> Circuit` (no inyecta `Component.pins` — `constraint_engine` resuelve pinout vía `component_registry`). **7 callsites migrados** a `tools.eda.constraint_engine.run_drc(dict_to_ir(d))`: `agent/agents/circuit_agent.py` ×3 (capa2 DRC, flujo LLM, review pass), `api/routers/circuits.py` ×2, `eval/test_full_integration.py`, `tools/pdf_exporter.py`. **Insight clave**: en el motor IR, `run_drc()` ejecuta las reglas de pinout (`PIN_INVALID` / `PIN_FORBIDDEN`) dentro del mismo paso → el patrón legacy `run_drc(d) + validate_pinout(d)` colapsa en una sola llamada. Los 3 sitios complejos en `circuit_agent.py` requirieron refactor (no find-replace) con helper `_run_drc_and_pinout` que extrae los pinout warnings filtrando `i.code in {"PIN_INVALID", "PIN_FORBIDDEN"}` y preserva el prefijo `[Pinout]` para consumidores downstream. **Tests nuevos**: `tests/_golden_fixtures_dict.py` con 3 dicts gemelos a los IR golden (formato `CircuitSynthesizer`), `tests/test_dict_to_ir.py::TestDictToIRStructural` (paridad estructural — `TestDRCParity` retirado tras delete del legacy, ya cumplió su función durante la migración). **Decisiones de borde**: `_vcc` del dict legacy se descarta (`CircuitMetadata` es `extra="forbid"`, no tiene field `vcc`); `net_class` del IR no se preserva en roundtrip (el dict del synthesizer no transporta el campo); el adapter falla fuerte (raise) ante input inválido. **Gap aceptado** `NO_DECOUPLING_CAP`: la nueva regla (`tools/eda/rules.py:169-184`) solo dispara si HAY caps en el circuito pero ninguno en VCC; la legacy disparaba siempre que un MCU no tenía caps. Diferencia intencional — la nueva regla es menos estridente para circuitos minimalistas (blink LED, motor driver). **Suite**: 359/359 passing post-delete. **Commits**: `e964e9a` (refactor + adaptador), `edcc092` (delete módulos). **Phase B BLOCKED**: retiro de `tools/component_pinouts.py` (1 callsite en `circuit_agent.py:10` + uso ~775) requiere primero poblar el registry con 11 componentes faltantes con datos completos (`drv8825, a4988, ne555, hc05, nrf24l01, servo, neopixel, ina219, hx711, lcd_i2c, pir`) — sin ellos los prompts LLM pierden warnings críticos que evitan hardware damage (ej: cap 100µF obligatorio en VMOT del DRV8825). El registry actual cubre 17/28 tipos legacy con datos completos (60.7%); migrar ahora degrada calidad de generación. Decisión: frenar Phase B hasta sprint dedicado a poblar registry. Mismatch adicional documentado: legacy hace fuzzy substring match (`"dht"` matchea `dht22`); registry hace lookup exacto por type/aliases — al desbloquear, requiere ampliar aliases YAML o cambiar lógica de matching.

> **2026-05-07 — v0.30.0 EDA refactor estructural (Fases 4-10 de 10) — pipeline determinista end-to-end:** completa el refactor de `instruc.txt`. **Fase 4 — Pin Allocator** (`tools/eda/pin_allocator.py`, ~280 LOC): API `allocate(circuit, requests) → AllocationResult`. `PinRequest(mcu_ref, net_name, function, bus_id, require_output, avoid)` — el LLM ya no decide GPIOs. Scoring determinista: +50 preferred_bus match, +5 GPIO simple, −10 strapping pin, −1 input_only en función no-input. Tiebreak alfabético numeric-aware (`GPIO2 < GPIO12`). Soporta multi-MCU, expansión wildcard PWM/ADC, conflict detection. Issues `PIN_NO_AVAILABLE / PIN_REQUEST_INVALID_MCU`. **Fase 5 — Placement Engine** (`tools/eda/placement_engine.py`, ~340 LOC): `place(circuit, options) → PlacementResult`. 6 zonas (`Zone` enum: MCU centro, POWER arriba-izq, COMM arriba-der, INPUT izq, OUTPUT der, PASSIVE abajo) con layout fraccional fijo. Clasificación por `registry.category` + overrides keyword (`_COMM_KEYWORDS / _OUTPUT_IC_KEYWORDS / _AC_KEYWORDS`). Tamaños por type override + fallback por categoría. Auto-grow del board (factor 1.25× hasta convergencia, max 8 iter). Pack row-major ordenado por ref → determinista. Snap to grid 2.54mm. AABB collision detection defensiva. Issues `ZONE_OVERFLOW / PLACEMENT_OVERLAP / ZONE_UNKNOWN_FALLBACK`. **Fase 6 — Routing Engine** (`tools/eda/routing_engine.py`, ~280 LOC): `route(circuit, options) → RoutingResult`. MST por net (Prim determinista, peso = Manhattan, tiebreak alfabético en `(peso, ref_a, ref_b)`). Por arista, prueba 2 L-shapes (H-then-V / V-then-H), elige la de 0 cruces en F.Cu; si no, B.Cu con vías auto en endpoints; si tampoco, fallback al de menor cruce. `_net_class_inferred` infiere class por keyword (vcc/5v/3v3 → power; gnd → ground=power) — el `net_class` explícito tiene prioridad. Trace width según class. Issues `NET_NODE_UNPLACED / ROUTE_UNRESOLVABLE`. **Fase 7 — Renderers Puros** (`tools/eda/render/`, ~600 LOC): `render_schematic_svg(circuit)` y `render_pcb_svg(circuit)` consumen Circuit IR ya con placement + traces + vias. **NO** deciden placement, routing, ni infieren nets — solo dibujan. SVG builder mínimo (`svg.py`) con atributos en orden alfabético → output byte-determinista. Schematic: marco + title block, wires por centroid+L-shape, símbolos rectangulares con ref/value, junctions ≥3 nodos. PCB: edge cuts, copper B.Cu→F.Cu, vías con drill, footprints (courtyard + body + silk), estilos dark (FR4 verde) y light (KiCad-style). **Fase 8 — Exporters desacoplados** (`tools/eda/export/`): `export_kicad_sch(circuit) → str` (.kicad_sch S-expr v6+ con `lib_symbols` inline custom + symbol instances + wires + net labels), `export_kicad_pcb(circuit) → str` (.kicad_pcb v6+ con layers + net table + footprints SMD/THT + segments + vías + Edge.Cuts), `render_bom_csv(circuit) → str` (BOM agrupado por (type, value, footprint)). UUIDs SHA-256 derivados del seed → byte-determinista. Builder S-expr en `_sexpr.py`. **Fase 9 — Golden tests visuales** (`tests/test_eda_golden.py` + `tests/_golden_fixtures.py` + `tests/golden_snapshots/`): 3 fixtures canónicas (blink_led_esp32, i2c_oled_dht22_esp32, motor_l298n_arduino). Pipeline completo serializa: components + placements + nets + traces summary + vias summary + DRC pre/post + zones + board + hashes (SHA-256) + byte_lengths de SVG/KiCad/BOM. Snapshots auto-creados primer run, comparados en runs siguientes. Tests adicionales de idempotencia y "no PIN errors residuales después del pipeline". **Fase 10 — Pipeline wrapper + dep marks** (`tools/eda/pipeline.py`): `run_pipeline(raw, options) → PipelineResult` wirea pre_drc → allocate → place → route → post_drc → render → export end-to-end. Cada etapa toggleable. `PipelineResult.has_errors()` agrega todas las severities. Deprecation notes en `tools/electrical_drc.py`, `tools/mcu_pinout_validator.py`, `tools/component_pinouts.py` apuntando al nuevo pipeline (mantienen compat operacional, retiro físico cuando el agent consuma IR). **Bug fix routing**: tiebreak en `_choose_path_and_layer` comparaba tuplas con `Vec2` (no orderable) — reemplazado por `(crossings, layer_priority, candidate_idx)`. **Tests**: 68 (Fases 1-3) + 20 pin_allocator + 24 placement + 21 routing + 21 render + 23 export + 9 golden + 7 pipeline = **193 tests EDA nuevos** (314 total con baseline). Suite full: **313 passed**, 3 fail / 21 errors pre-existentes — cero regresión. **Arquitectura final**: ~3000 LOC nuevos en `tools/eda/{ir, component_registry, constraint_engine, rules, pin_allocator, placement_engine, routing_engine, render, export, pipeline}.py`. Pipeline 100% determinista (mismo input → byte-equivalent output verificado por golden hashes). La IA ya solo hace reasoning — toda la ingeniería EDA es determinista. **Next**: integrar el pipeline al agent (orchestrator → run_pipeline) y retirar legacy renderers cuando se valide end-to-end.

> **2026-05-07 — v0.29.0 EDA refactor estructural (Fases 1-3 de 10):** primer tramo del refactor pedido en `instruc.txt` — eliminar el enfoque de fixes locales y migrar a una arquitectura EDA determinista LLM → IR → Validation → Constraints → Pin Alloc → Placement → Routing → Renderer → Exporters. **Fase 1 — Circuit IR** (`tools/eda/ir/`): pydantic v2, modelos `Circuit / Component / Pin / Net / Node / Trace / Via / Board / DesignRules / Constraint / ValidationIssue / CircuitMetadata / PlacementInfo / Footprint / Symbol`, enums `ElectricalType / Side / Layer / Severity`, `Vec2` frozen. Validación estructural (refs únicos, nodos→componentes existen, no duplicados de pin/net), roundtrip JSON limpio (`to_json` / `from_json`), `extra="forbid"` en todos los modelos. **Fase 2 — Component Registry** (`tools/eda/component_registry/`): YAML como fuente única de verdad. Schema pydantic (`ComponentSpec / PinSpec / MCUSpec / BusPins / VoltageSpec / WiringRequirement`) + `Registry` con lookup exacto por `type` y por alias case-insensitive + singleton lru_cached (`get_registry()`). 17 archivos YAML cubren: 6 MCUs (esp32, arduino_uno/nano/mega, esp8266, raspberry_pi_pico, stm32 con `forbidden_pins / input_only_pins / boot_strapping_pins / adc_pins / pwm_pins / preferred_buses` + pinout completo), 7 sensores (dht11/22, bmp280, hc_sr04, mpu6050, fc28, ds18b20), 3 power (lm7805, lm317, ams1117), l298n, relay_module, oled_ssd1306, ds3231, 5 pasivos (resistor, capacitor, capacitor_electrolytic, led, diode), fuse, screw_terminal. Cubre los tipos referenciados por DRC y la mayoría de `_TYPE_TO_FOOTPRINT` críticos. **Fase 3 — Constraint Engine** (`tools/eda/constraint_engine.py` + `rules.py`): engine declarativo con `ValidationContext` (snapshot indexado: `comp_by_ref / spec_by_ref / refs_in_net / nets_of_ref / refs_by_category` — todo O(1) para reglas), `ConstraintRule` + `RuleRegistry` con decorator `@rule_registry.register("CODE")`, `validate(circuit, rules=None)` y `run_drc(circuit)` (API compat con `tools/electrical_drc.py` legacy — devuelve dict). 19 reglas registradas: las 18 de `electrical_drc.py` migradas (NO_POWER_NET, SHORT_CIRCUIT, LED_WITHOUT_RESISTOR, ISOLATED_COMPONENT, DUPLICATE_NET_NODE, NO_DECOUPLING_CAP, NO_I2C_PULLUP, NO_ONEWIRE_PULLUP, HIGH_CURRENT_NO_FUSE, VOLTAGE_MISMATCH, MISSING_RESET_CAP, OVERCURRENT_PIN, SIGNAL_5V_ON_3V3_GPIO, MOTOR_DIRECT_TO_MCU, ESP_WIFI_NO_BULK_CAP, MCU_MISSING_POWER, RELAY_FLYBACK, AC_CONNECTOR_NO_FUSE) + 3 que reemplazan `mcu_pinout_validator.py` consumiendo el registry (`PIN_INVALID / PIN_FORBIDDEN / PIN_INPUT_ONLY_MISUSE`). **Tests**: +68 nuevos pasando (`test_eda_ir.py` 20, `test_eda_component_registry.py` 18, `test_eda_constraint_engine.py` 30). Suite full: **188 passed**, 3 fail / 21 errors (los pre-existentes de `electrical_formulas` y `versioning_sharing` — cero regresión sobre baseline 120). **Compat**: `tools/electrical_drc.py` y `tools/mcu_pinout_validator.py` no se tocaron — siguen importándose desde `circuit_agent.py` igual. La migración a usar el nuevo engine ocurre cuando los renderers/exporters consuman IR (Fases 7-10). **Decisiones**: pydantic v2 (no dataclasses) por validación gratis + JSON schema export; YAML (no JSON) por legibilidad de 50+ componentes; match exacto en `Registry.get` (no substring) — alias laxos como "c"/"r" matcheaban cualquier string; `extra="forbid"` en todo schema para detectar typos en YAML al startup. **Deps**: `requirements.txt` + `pydantic>=2.6,<3.0` + `PyYAML>=6.0,<7.0`. **Próximos pasos**: Fase 4 Pin Allocator (consume registry + IR para asignar GPIOs deterministas), Fase 5 Placement Engine, Fase 6 Routing Engine.

> **2026-05-06 — v0.28.0 EDA renderer overhaul:** schematic con hoja A4 ISO 7200 (frame + zone refs A-D / 1-8 + title block esquina inf-der), layout zonal proporcional al área útil, Manhattan-tree wire routing + collision-aware net labels. PCB con footprints reales por familia (MCU module / sensor / relay / TO-220 / TO-92 / axial / radial / generic), side panel off-board (PCB INFO + DRC + legend + stats), `_board_size` sumando anchos reales de zonas (fix overflow OLED). 3D viewer reemplaza breadboard por PCB FR4 verde con edge.cuts dorado + 4 mounting holes M3 + silkscreen STRATUM PCB + fiducials. Tests EDA 55/55, baseline 120 passing.
...
│           ├── live_circuit.js     # Live Hardware State Visualizer (v0.2.0)
...
│   ├── eda/                        # Renderer interno separado en 4 responsabilidades (v0.27.0)
...
✅ **Nuevo diseño CAD-instrument** (v0.0): design system completo con `panel-cnr`, `ruler`, `msg-user`/`msg-agent` diferenciados, acento azul cyan.
...
### 4.13 Platform Context Persistente (v0.0)
...
### 4.14 Firmware Iterativo con Diff (v0.0 / fix v0.0.1)
...
⚠️ Fix v0.0.1: `"modify"` estaba ausente de la tupla de intents válidos en `_classify_intent()` — el LLM lo clasificaba bien pero el resultado era descartado. Además MODIFY_KEYWORDS se evaluaba después de DESIGN_KEYWORDS. Ambos bugs corregidos en `hardware_agent.py`.

### 4.15 Datasheet Auto-Fetch (v0.0)
...
### 4.17 Generación de Circuitos con Dominio Detectado (v0.1 / fix v0.5.0)
...
⚠️ Fix v0.5.0: `domain_hint` se calculaba pero nunca se pasaba al `.format()` del `CIRCUIT_PARSE_PROMPT` → circuito de riego salía sin sensor de humedad de suelo (FC-28/YL-69). Fix: `{domain_hint}` agregado al template y `domain_hint=domain_hint` al format call.
⚠️ Fix v0.5.0: Agregada regla `CRÍTICO: cada nodo en UN SOLO net, nunca repetido` al prompt → elimina warnings de nodos duplicados entre nets.
...
### 4.18 SchematicRenderer Mejorado (v0.1 / fix v0.5.0)
...
⚠️ Fix v0.5.0: SVG generado con `size=(1000, 700)` fijo → el browser lo mostraba centrado sobre fondo gris al abrir `/api/circuits/{id}/schematic.svg` directamente. Fix: `size=('100%', '100%')` con `viewBox="0 0 1000 700"` → SVG responsivo que llena toda la pantalla.

### 4.19 KiCad Export — .kicad_sch (v0.1)
...
### 4.30 MCU Pin Assignment Rules (v0.7.0)
...
### 4.31 SchematicRenderer — 15+ símbolos nuevos (v0.7.0)
...
### 4.32 Electrical DRC — 3 checks nuevos (v0.7.0)
...
### 4.33 BOM Grouping + KiCad Footprints (v0.7.0)
...
### 4.34 Firmware — Retry 2x + Error Parsing inteligente (v0.7.0)
...
### 4.35 Firmware Snippet Library (v0.7.0)
...
### 4.36 PCB Renderer — Mejoras visuales profesionales (v0.7.0)
...
### 4.39 3D Viewer Lighting + Schematic Dispatch Fix (v0.9.0)
...
### 4.38 EDA Visualization — Light Theme KiCad-style (v0.8.0)
...
### 4.40 EDA Renderer — Upgrade Profesional KiCad-level (v0.19.0)
...
### 4.20 PCBRenderer Mejorado (v0.1)
...
### 4.29 Circuit Viewer Profesional — 2D + 3D (v0.5.0)
...
### 4.28 Verificación total y hardening (v0.4.1)
...
### 4.26 Editor Visual de Circuitos (v0.4 / actualizado v0.5.0)
...
⚠️ v0.5.0: click-to-select migrado de elementos SVG (ya no existen en la vista server-side) a los items de la lista de componentes del sidebar. Cada item tiene `data-comp-id` y llama `_selectComponent()` al hacer clic. `_wireEditorClicksOnList()` se llama al final de `renderComponentsList()`.

### 4.27 Tests Automatizados pytest (v0.4)
...
### 4.22 Versioning de Circuitos (v0.3)
...
### 4.23 Share via link público (v0.3)
...
### 4.24 Firmware Production-Ready (v0.3)
...
Los 3 failures + 21 errors son **pre-existentes** (documentados en el changelog v0.27.0). En 2026-05-06 se eliminó el módulo dead `agent/keywords/circuit_keywords.py` y su test (`test_keywords.py`, 4 tests) — el conteo bajó de 124 a 120 sin regresiones.
...
## 11. PERFORMANCE — v0.6.0 (2026-04-22)
...
## 12. CODE QUALITY PASS — v0.11.0 (2026-04-24)
...
> **Prioridad para agregar tests** (retorno más alto): `tools/electrical_formulas.py` (puras, 0 deps, cubrirían 25 fórmulas con ~30 tests), `database/sql_memory.py` (conftest.py ya tiene `tmp_db`), `llm/cache.py` (O(1) dict + TTL lógica).
...
## 19. FIXES MEDIO PRIORITY — v0.23.0 (2026-04-30)
...
| `agent/agent_controller.py` | `import asyncio` debe estar a nivel módulo (corregido en v0.11.0) — cualquier nuevo método que use `asyncio.*` lo requiere | Monitorear en nuevos métodos |
